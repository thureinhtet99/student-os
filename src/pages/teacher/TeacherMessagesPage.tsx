...
export default function TeacherMessagesPage() {
  ...
  const [to, setTo] = useState(users.find((u) => u.role === 'parent')?.id ?? '')
    ...
    <div className="space-y-6">
      <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Chat with parents, students, and colleagues.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <Panel title="Inbox" description={`${myMessages.filter((m) => !m.read).length} unread`} className="lg:col-span-1">
